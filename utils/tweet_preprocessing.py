...
def simpleTokenize(text):
    ...
    for i in range(len(bads)):
        ...
        if re.compile(url).match(bads[i][0]):
            zippedStr = addAllnonempty(zippedStr, [''])
...
def tokenizeRawTweetText(text):
    ...
    prepro_text = ' '.join(tokens).lower()
    prepro_text = re.sub('&', 'and', prepro_text).strip()
    prepro_text = re.sub('(^rt)|( rt$)', '', prepro_text).strip()
    prepro_text = re.sub('( rt )', ' ', prepro_text).strip()
    prepro_text = re.sub('(http : ...)|(http)', '', prepro_text).strip() # remove http
    prepro_text = re.sub("(^::?)|(:?:$)", "", prepro_text).strip()

    prepro_text = re.sub('[ ]{2, }',' ', prepro_text) #remove extra space
